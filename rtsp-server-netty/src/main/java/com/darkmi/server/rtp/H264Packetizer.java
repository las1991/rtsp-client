...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	public void run() {
		...
		logger.info(TAG,"H264 packetizer started !");
		stats.reset();
		count = 0;
		...
		try {
			while (!Thread.interrupted()) {
				...
				delay = stats.average();
				//logger.info(TAG,"duration: "+duration/1000000+" delay: "+delay/1000000);

			}
		} catch (IOException e) {
		} catch (InterruptedException e) {}

		logger.info(TAG,"H264 packetizer stopped !");
	...
	private void send() throws IOException, InterruptedException {
		...
		if (streamType == 0) {
			...
			if (!(header[0]==0 && header[1]==0 && header[2]==0)) {
				...
				logger.error(TAG, "NAL units are not preceeded by 0x00000001");
				streamType = 2; 
				return;
			}
		} else {
		...
		if (type == 7 || type == 8) {
			logger.info(TAG,"SPS or PPS present in the stream.");
			count++;
			if (count>4) {
				sps = null;
				pps = null;
		...
		//logger.info(TAG,"- Nal unit length: " + naluLength + " delay: "+delay/1000000+" type: "+type);
		...
		if (naluLength<=MAXPACKETSIZE-rtphl-2) {
			...
			//logger.info(TAG,"----- Single NAL unit - len:"+len+" delay: "+delay);
		}
		// Large NAL unit => Split nal unit 
			...
			while (sum < naluLength) {
				...
				//logger.info(TAG,"----- FU-A unit, sum:"+sum);
			}
		}
	}
	...
	private void resync() throws IOException {
		...
		logger.error(TAG,"Packetizer out of sync ! Let's try to fix that...(NAL length: "+naluLength+")");
		...
		while (true) {
			...
			if (type == 5 || type == 1) {
				...
				if (naluLength>0 && naluLength<100000) {
					oldtime = System.nanoTime();
					logger.error(TAG,"A NAL unit may have been found in the bit stream !");
					break;
				}
				if (naluLength==0) {
					logger.error(TAG,"NAL unit with NULL size found...");
				} else if (header[3]==0xFF && header[2]==0xFF && header[1]==0xFF && header[0]==0xFF) {
					logger.error(TAG,"NAL unit with 0xFFFFFFFF size found...");
				}
			}
