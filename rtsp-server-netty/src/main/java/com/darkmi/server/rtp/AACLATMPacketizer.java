...
public class AACLATMPacketizer extends AbstractPacketizer implements Runnable {
	...
	public void run() {
		...
		logger.info(TAG,"AAC LATM packetizer started !");
		...
		try {
			while (!Thread.interrupted()) {
				...
				if (length>0) {
					...
					//logger.info(TAG,"length: "+length+" ts: "+bufferInfo.presentationTimeUs);
					oldts = ts;
			...
			logger.error(TAG,"ArrayIndexOutOfBoundsException: "+(e.getMessage()!=null?e.getMessage():"unknown error"));
			e.printStackTrace();
		} catch (InterruptedException ignore) {}

		logger.info(TAG,"AAC LATM packetizer stopped !");
