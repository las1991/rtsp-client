...
public final class Media extends Field {
    ...
    Media(String type, int port, int count, String protocol, int... formats) {
        this.type = Strings.emptyToNull(StringUtils.trim(type));
        Assert.assertNotNull("type may not be empty", type);
        this.port = port > 0 && port < 65535 ? port : 0;
        this.count = Math.max(0, count);
        this.protocol = Strings.emptyToNull(StringUtils.trim(protocol));
        Assert.assertNotNull("protocol may not be empty", protocol);
    ...
    @Override
    public int hashCode() {
        return Objects.hashCode(type, port, count, protocol, formats);
    }
